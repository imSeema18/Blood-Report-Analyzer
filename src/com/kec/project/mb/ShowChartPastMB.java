...
import com.kec.project.model.GraphsRec;
import com.kec.project.model.Report;



@SessionScoped
@ManagedBean(name = "showChartPastMB")
public class ShowChartPastMB implements Serializable {
...
int uid;
	...
	GraphsRec records = new GraphsRec();
	...
	public void checkTpl(int uid) {
		...
		int tId = 1;
		newRangeTpl = charts.getCompInfoPast(uid,tId);
		if (newRangeTpl.size() > 2) {
			records.setTplRec(true);
			createLineModelsTpl(uid, newRangeTpl);
		} else {
			records.setTplRec(false);
	...
	public void checkAgr(int uid) {
		...
		newRangeAgr = charts.getCompInfoPast(uid,tId);
		if (newRangeAgr.size() > 2) {
			records.setAgrRec(true);
			createLineModelsAgr(uid, newRangeAgr);
		} else {
			records.setAgrRec(false);
		...
		public void checkUric(int uid) {
			...
			newRangeUric = charts.getCompInfoPast(uid,tId);
			if (newRangeUric.size() > 2) {
				records.setUricRec(true);
				createLineModelsUric(uid, newRangeUric);
			} else {
				records.setUricRec(false);
	...
	public void checkRBC(int uid) {
		...
		newRangeRBC = charts.getCompInfoPast(uid,tId);
		if (newRangeRBC.size() > 2) {
			records.setRbcRec(true);
			createLineModelsRBC(uid, newRangeRBC);
		} else {
			records.setRbcRec(false);
	...
	public void checkPlt(int uid) {
		...
		newRangePlt = charts.getCompInfoPast(uid,tId);
		if (newRangePlt.size() > 2) {
			records.setPltRec(true);
			createLineModelsPlt(uid, newRangePlt);
		} else {
			records.setPltRec(false);
	...
	public ChartsEJB getCharts() {
		return charts;
	}


	public void setCharts(ChartsEJB charts) {
		this.charts = charts;
	}


	public GraphsRec getRecords() {
		return records;
	}


	public void setRecords(GraphsRec records) {
		this.records = records;
	}


	public void setNewModelAgr(LineChartModel newModelAgr) {
		this.newModelAgr = newModelAgr;
	}
